...
erDiagram
    ...
    USERS {
        ...
        varchar last_name
        varchar phone
        bigint immediate_supervisor_id FK "Jefe inmediato, NULL para root"
    ...
    USER_TENANTS {
        ...
        bigint user_id FK
        bigint tenant_id FK
        boolean is_primary "Tenant principal del usuario"
        timestamp created_at
        timestamp updated_at
    }
    ...
    VACATION_REQUESTS {
        ...
        boolean was_taken "Indica si fueron tomadas, default TRUE al aprobar"
        timestamp created_at
        timestamp updated_at
    }
...
- ✅ Multi-tenancy: usuarios pueden pertenecer a múltiples organizaciones (tabla `user_tenants`)
- ✅ Aprobación de vacaciones: cada usuario tiene su jefe inmediato (`immediate_supervisor_id`)
- ✅ Vacaciones tomadas: campo `was_taken` para mapear si fueron tomadas (default TRUE al aprobar)
...
CREATE TABLE users (
    ...
    phone VARCHAR(20) NULL,
    immediate_supervisor_id BIGINT UNSIGNED NULL COMMENT 'Jefe inmediato para aprobación de vacaciones',
    status ENUM('active', 'inactive', 'terminated', 'pending') NOT NULL DEFAULT 'pending',
    created_at TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    deleted_at TIMESTAMP NULL DEFAULT NULL,
    FOREIGN KEY (immediate_supervisor_id) REFERENCES users(id) ON DELETE SET NULL,
    INDEX idx_supervisor (immediate_supervisor_id),
    INDEX idx_status (status),
...
### 5. `user_tenants` (Usuarios en Múltiples Organizaciones)
...
CREATE TABLE user_tenants (
    ...
    is_primary BOOLEAN NOT NULL DEFAULT FALSE COMMENT 'Organización principal del usuario',
    created_at TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
...
### 6. `document_types` (Tipos de Documentos)
...
CREATE TABLE vacation_requests (
    ...
    was_taken BOOLEAN NOT NULL DEFAULT FALSE COMMENT 'Indica si las vacaciones fueron tomadas. Se marca TRUE al aprobar',
    created_at TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (tenant_id) REFERENCES tenants(id) ON DELETE CASCADE,
...
INSERT INTO users (email, password, document_type, document_text, name, last_name, phone, status, email_verified_at)
VALUES (
    'admin@demo.com',
    '$2y$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewY5GyJN0lf5eRpW',
    'dni',
...
WHERE u.email = 'admin@demo.com' AND t.ruc = '20123456789';
...
-- Crear empleado cliente (con jefe inmediato = admin)
INSERT INTO users (email, password, document_type, document_text, name, last_name, phone, immediate_supervisor_id, status, email_verified_at)
SELECT 
    'empleado@demo.com',
    '$2y$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewY5GyJN0lf5eRpW',
    'dni',
...
-- Asociar empleado al tenant
INSERT INTO user_tenants (user_id, tenant_id, is_primary)
SELECT u.id, t.id, TRUE
FROM users u
CROSS JOIN tenants t
WHERE u.email = 'empleado@demo.com' AND t.ruc = '20123456789';
...
2. **Relaciones:** `user_roles` → `user_tenants` → `document_types`
3. **Documentos:** `documents`
4. **Otros:** `vacation_requests` → `notifications` → `audit_logs`
5. **Datos iniciales:** 
   - Roles → Document Types 
   - Root User 
   - Tenant de prueba 
   - Admin y asignar a tenant (user_tenants)
   - Empleado con supervisor y asignar a tenant (user_tenants)

**Nota:** Laravel Migrations creará estas tablas automáticamente siguiendo este orden.

**Cambios importantes:**
- ✅ Usuarios pueden pertenecer a múltiples organizaciones (tabla `user_tenants`)
- ✅ Campo `tenant_id` eliminado de `users` (ahora en `user_tenants`)
- ✅ Campo `is_vacation_approver` eliminado (reemplazado por `immediate_supervisor_id`)
- ✅ Campo `immediate_supervisor_id` en `users` (aprobación automática por jefe)
- ✅ Campo `was_taken` en `vacation_requests` (mapear vacaciones no tomadas)
- ✅ Campo `approved_at` ya existía en `vacation_requests` (confirmación timestamp)
