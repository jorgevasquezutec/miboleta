...
class PasswordService
{
    ...
    public function adminResetPassword(
        ...
        // Send notification email (queued via Horizon)
    ...
    {
        Log::info('[PasswordService] Queueing admin reset email', [
            'user_id' => $user->id,
            'email' => $user->email,
        ]);

        // El Mailable implementa ShouldQueue, se enviará en background via Horizon
        ...
        Log::info('[PasswordService] Admin reset email queued successfully', [
            'user_id' => $user->id,
            'email' => $user->email,
        ]);
    }
